...
import { calcCenter, calcDiff, calcMin, createCanvasFontString, ensureBetween, formatValue, toPoint } from './utils'
...
export class Painter {
  ...
  private setTransform(style: ITransform) {
    this._create((ctx) => {
      let {
        transform,
        angle = 0,
      ...
      if (!transform) {
        ...
        const e = origin.x // x方向的平移，即变换的原点
        const f = origin.y // y方向的平移，即变换的原点
  ...
  rect(x: number, y: number, w: number, h: number, style: RectStyle = {}) {
    return this._create((ctx) => {
      ...
      this.setTransform({
        ..._style,
        anchor: {
          x: x * (1 - toPoint(_style.scale || 1).x),
          y: y * (1 - toPoint(_style.scale || 1).y),
        },
      })
   ...
   * @param save
   */
  private _create<T extends (ctx: CanvasRenderingContext2D) => any>(callback: T, save = true): ReturnType<T> {
    if (!this._checkCtx()) {
